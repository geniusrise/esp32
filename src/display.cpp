...
Display::displayBitmap(File& bitmapFile)
{
  ...
  uint32_t buffer[bitsPerPixel / 8];
  uint32_t pixelCount = width * height;

  if (bitsPerPixel == 32) {
    for (int32_t y = height - 1; y >= 0; y--) {
      for (uint32_t x = 0; x < width; x++) {
        bitmapFile.read((uint8_t*)buffer, 4);
        uint32_t color = buffer[0];
        uint16_t convertedColor = ((color & 0xF80000) >> 8) |
                                  ((color & 0x00FC00) >> 5) |
                                  ((color & 0x0000F8) >> 3);
        tft.drawPixel(x, y, convertedColor);
      }
    }
  } else {
    // 16-bit RGB565 bitmap
    while (pixelCount--) {
      bitmapFile.read((uint8_t*)buffer, 2);
...
//         uint8_t a = rowData[x * 4 + 3]; // Alpha channel (unused in this
//         case) uint16_t color = ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >>
